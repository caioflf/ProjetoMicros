...
unsigned short calcula_distancia(unsigned short x, unsigned short y, unsigned short x_final, unsigned short y_final){	// calcula a distancia de um trajeto
	unsigned short distancia = modulo(x-x_final)+ modulo(y-y_final);
	return distancia;
}

void gps (unsigned short x, unsigned short y, unsigned short x_final, unsigned short y_final, unsigned char flagComCliente){  		// indica o sentido que o carro deve seguir
	...
	if (x != x_final && y != y_final){
		...
		comando_lcd(0xC0);
		escreve_lcd("Em ");
		imprimeASCII(modulo(posCarroGlobal.y-y_prox + posCarroGlobal.x-x_prox));
		escreve_lcd("m ");
		if (sentido == 'N'){
			escreve_lcd("Norte.");
			} else if (sentido == 'S'){
...
unsigned short estimagemPreco (unsigned short dist) {		// preço em centavos
...
unsigned char verifica_login(){			//scan apenas da linha 4, com um "debounce" maior para verificar se apertou a tecla tempo suficiente
	...
	if(!verificacao_tecla2(4)){ // lembrar de mudar :D
		PORTD|=(1<<(3));
		return teclado[3][2];
	}
	PORTD|=(1<<(3));
...
void ordenaClientes (cliente *clientesEspera, char opcaoB, char *quantidadeClientes){
	...
	if (opcaoB == 1){														// ordena de acordo com a distCliente
		...
		for (i = 0; i<*quantidadeClientes-1; i++){
			for(j = 0; j < *quantidadeClientes - i; j++){
...
void printDirecao(unsigned char flagEmCorrida, unsigned flagComCliente, cliente clienteAtual) {	
	if (!flagEmCorrida){
		limpa_lcd();
	...
	if (!flagComCliente) {
		gps(posCarroGlobal.x, posCarroGlobal.y, clienteAtual.pos_saida_x, clienteAtual.pos_saida_y, flagComCliente);
		return;
	}	
	gps(posCarroGlobal.x, posCarroGlobal.y, clienteAtual.pos_destino_x, clienteAtual.pos_destino_y, flagComCliente);
		return;
}

void menu(char *indiceCliente, char *indiceInfo, char quantidadeClientes, cliente *clientesEspera, char flagComCliente, char estadoMotorista, cliente clienteAtual, char flagEmCorrida, char letra, char i){	
		if (letra == '8'){
			...
			if (*indiceCliente == 1){			// ja estava mostrando o cliente no topo, agora mostra a direção a ser seguida
				*indiceCliente-=1;
				*indiceInfo = 0;
				printDirecao(flagEmCorrida, flagComCliente, clienteAtual);
				return;
			}
			*indiceCliente-=1;
			*indiceInfo = 0;
...
void aceitaCorrida (char *indiceCliente, cliente *clienteAtual, cliente *clientesEspera, char *estadoMotorista, char *motoristaOcupado, char *flagEmCorrida, char *quantidadeClientes, char letra){
	...
	if (letra == '#'){										// se ele nao esta em atendimento, e aceitou o cliente
		...
		atraso_500ms();										// aguarda resposta do servidor
		if (flagClienteDisponivel == 2){					//se servidor responde que cliente aceitou
			...
			*flagEmCorrida = 1;										// levanta a flag que esta em atendimento
			letra = '\0';											// para evitar entrar em outra funcao
			limpa_lcd();
			escreve_lcd("Cliente:");
	...
	if (letra == '*'){
		...
		*quantidadeClientes = *quantidadeClientes - 1;
		removeCliente(clientesEspera, quantidadeClientes, indiceCliente);	// funcao para reordenar
		return;
	}
	return;
}
void acaoPassageiro(char *estadoMotorista, cliente clienteAtual, char *flagComCliente, char *flagEmCorrida, char letra){
	if (*flagEmCorrida){					// se ele esta em corrida
		if (letra == '#'){
			if (*flagComCliente == 0){		//e sem o cliente
				*flagComCliente = 1;		//pega o cliente
				string_serial("UI");
...
char ubergs(char *flagSistema, char *opcaoB, char *motoristaOcupado, char *estadoMotorista, char flagPerfil, cliente *clientesEspera, char *quantidadeClientes){
	...
	char flagComCliente = 0;			// flag que indica se o motorista esta com cliente
	char flagEmCorrida = 0;				// flag que indica se o motorista esta em atendimento 
	...
	*estadoMotorista = 1;
	...
	while (1){	
		...
		menu(&indiceCliente, &indiceInfo, *quantidadeClientes, clientesEspera, flagComCliente, estadoMotorista, clienteAtual, flagEmCorrida, letra, i);
		aceitaCorrida(&indiceCliente, &clienteAtual, clientesEspera, estadoMotorista, motoristaOcupado, &flagEmCorrida, quantidadeClientes, letra); 		
		movimento_manual(letra);
		acaoPassageiro(estadoMotorista, clienteAtual, &flagComCliente, &flagEmCorrida, letra);
...
char login (char *flagSistema, char *opcaoB, char *motoristaOcupado, char *estadoMotorista){
	...
	char aux = 0;
	for (aux = 0; aux < MAX_CLIENTES; aux++){
...
void interpreta_serial(){ //interpreta as mensagens enviadas pelo servidor externo
	unsigned char i;
	if (contador_global > 12 || serial_global[0] != 'S'){
		limpa_serial_global();
	} 
